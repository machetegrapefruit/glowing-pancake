...
public class BabiWriterTask implements Runnable {
	private ConcurrentLinkedQueue<BabiRecommendation> recQueue;
	private String outputPath;
	private boolean finished = false;
	public BabiWriterTask(ConcurrentLinkedQueue<BabiRecommendation> recQueue, String outputPath) {
			...
			while (!finished) {
				if (recQueue.isEmpty()) {
					...
					while (!recQueue.isEmpty()) {
						BabiRecommendation current = recQueue.poll();
						gson.toJson(current, BabiRecommendation.class, jWriter);
						added++;
					}
					if (added > 0) {
