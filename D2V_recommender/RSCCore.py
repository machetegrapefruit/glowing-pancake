    ...
    if append:
        with open("Dataset/MovieInfo.csv", "a", newline="", encoding="utf8") as csvfile:
            ...
            writer.writerow(
                [
                    __films_IDs__[index],
                    __films_titles__[index],
                    __tokenized_plots__[index],
                    __films_genres__[index],
                    __films_cast__[index],
                    __films_directors__[index],
                ]
            )
            csvfile.close()
            return 200
...
def select_model(selected_model):
    ...
    try:
        if (
            __tokenized_plots__ is not None
            and __films_IDs__ is not None
            and __films_titles__ is not None
        ):
            print(
                "Movie Info Already Loaded"
            )  # Già caricati in memoria le informazioni sui film
        else:
            raise Exception
    except Exception:
        (
            ...
            __films_genres__,
            __films_directors__,
        ) = __tonkens_from_documents_gensim__()
    ...
    if selected_model == 1 or selected_model == 2:
        ...
        try:
            ...
            thread = threading.Thread(
                target=__d2v__.load_model,
                args=(
                    __tokenized_plots__,
                    "Models/Doc2Vec/doc2vec_model",
                    __returned_queue__,
                ),
            )
            thread.start()
    ...
    if selected_model == 5 or selected_model == 6:
        ...
        if selected_model == 5:
            ...
            try:
                ...
                __fasttext__ = None
                thread = threading.Thread(
                    target=__ft__.create_model_fasttext_fb,
    ...
    if selected_model == 7:
        ...
        try:
            if (
                __tfidf_model__ is not None
                and __tfidf_index__ is not None
                and __tfidf_dictionary__ is not None
            ):
                print("Already Loaded")
            else:
                raise Exception
...
def get_suggestions_from_sentence(sentences, evaluate_sim_word, pref_entity):
    ...
    if len(sentences) > 0:
        recommends_from_senteces = __get_suggestion_from_sentence__(
            senteces=sentences, evaluate_sim_word=evaluate_sim_word
        )
    else:
        for i in range(len(__films_IDs__)):
            recommends_from_senteces.append(
                {"Rank": i + 1, "ID": __films_IDs__[i], "Value": 0}
            )
    list_value = []
    list_IDs = []
    value_cos_temp = list()
    ...
    if len(pref_entity) > 0:
        ...
        recommends_from_entity = __get_suggestion_from_entity__(
            pref_entity,
            films_IDs=__films_IDs__,
    ...
    if len(value_cos_temp) == 0:
        for i in range(len(__films_IDs__)):
            if recommends_from_senteces[i]["ID"] == recommends_from_entity[i]["ID"]:
                ...
                list_value.append(
                    recommends_from_senteces[i]["Value"]
                    + recommends_from_entity[i]["Value"]
                )
    else:
        for i in range(len(__films_IDs__)):
    ...
    for i in range(5):
        ...
        output_to_print.append(
            [i + 1, __films_titles__[__films_IDs__.index(IDs[i])], value[i], IDs[i]]
        )
    print("--------------" + str(__id_Model__) + "--------------")
    df = pd.DataFrame(
        output_to_print, columns=["rank", "title", "similarity_score", "ID"]
    )
...
def get_suggestion(
    preferences_IDs, pref_entity, movie_to_ignore, negative_entity, rec_list_size
):
    global __tokenized_plots__, __films_IDs__, __films_titles__, __films_cast__, __films_genres__, __films_directors__
    IDs_pref = list()
    tokenized_pref = list()
    ...
    for i in range(rec_list_size + len(IDs_pref) + len(movie_to_ignore)):
        ...
        output_to_print.append(
            [
                len(recommends_entity) + 1,
                __films_titles__[__films_IDs__.index(IDs[i])],
                value[i],
                IDs[i],
            ]
        )
    print("--------------" + str(__id_Model__) + "--------------")
    df = pd.DataFrame(
        output_to_print, columns=["rank", "title", "cosine_similarity", "ID"]
    )
    pd.set_option("display.max_rows", None, "display.max_columns", None)
    print(df)
    return recommends_entity
...
def __get_rec__(IDs_pref, tokenized_pref):
    ...
    if __id_Model__ == 1 or __id_Model__ == 2:
        ...
        recommends = __d2v__.get_recommendations_doc2vec(
            token_strings=tokenized_pref,
            documents=__tokenized_plots__,
            titles=__films_titles__,
            IDs=__films_IDs__,
            modelDoC=__doc2vec__,
            most_similar=__most_similar__,
            prefIDs=IDs_pref,
        )
        ...
        try:
            ...
            # Se il modello non è caricato lo prendiamo dalla cosa dei risultato. Aspetta che termini il thread
        recommends = __w2v__.get_recommendations_word2vec(
            token_strings=tokenized_pref,
            documents=__tokenized_plots__,
            titles=__films_titles__,
            IDs=__films_IDs__,
            modelWord=__word2vec__,
            pretrained=__w2c_pre_trained__,
            prefIDs=IDs_pref,
        )
        ...
        try:
            ...
            # Se il modello non è caricato lo prendiamo dalla cosa dei risultato. Aspetta che termini il thread
        recommends = __ft__.get_recommendations_fastText(
            token_strings=tokenized_pref,
            documents=__tokenized_plots__,
            titles=__films_titles__,
            IDs=__films_IDs__,
            modelFastText=__fasttext__,
            pretrained=__ft_pre_trained__,
            prefIDs=IDs_pref,
        )
    elif __id_Model__ == 7:
        global __tfidf_model__, __tfidf_index__, __tfidf_dictionary__
        try:
            ...
            # Se il modello non è caricato lo prendiamo dalla cosa dei risultato. Aspetta che termini il thread
        recommends = __tfidf__.get_recommendations_tfidf(
            token_strings=tokenized_pref,
            documents=__tokenized_plots__,
            titles=__films_titles__,
            IDs=__films_IDs__,
            dictionary=__tfidf_dictionary__,
            tfidfmodel=__tfidf_model__,
            index=__tfidf_index__,
            prefIDs=IDs_pref,
        )
    return recommends
        ...
        try:
            ...
            for sentence in senteces:
                ...
                for token in doc:
                    ...
                    if (
                        token.lemma_
                        in ["film", "movie", "like", "love", "appreciate", "I"]
                        or token.is_stop
                        or token.is_punct
                    ):
                        continue
...
def __get_suggestion_from_entity__(
    entities,
    films_IDs,
    films_cast,
    films_genres,
    ...
    for i, ID in enumerate(films_IDs):
        ...
        films_entities = numpy.concatenate(
            (films_cast[i], films_genres[i], films_directors[i])
        )
...
if __name__ == "__main__":
    ...
    get_suggestion(
        preferences_IDs=["Q172975", "Q26698156", "Q182254"],
        pref_entity=[],
        negative_entity=[],
        movie_to_ignore=[],
        rec_list_size=1,
