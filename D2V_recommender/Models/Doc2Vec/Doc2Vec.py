...
def create_model_doc2vec(documents, model_name):
    ...
    doc2vec = Doc2Vec(
        tagged_documents, vector_size=100, min_count=3, workers=8, epochs=100
    )
    # doc2vec.build_vocab(tagged_documents)
    doc2vec.train(
        tagged_documents, total_examples=doc2vec.corpus_count, epochs=doc2vec.epochs
    )
...
def get_recommendations_doc2vec(
    token_strings, documents, titles, IDs, modelDoC, most_similar, prefIDs
):
    recommend_movies = []
